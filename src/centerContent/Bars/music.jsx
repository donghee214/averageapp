...
export default class Sleep extends React.Component {
    ...
    componentDidMount() {
        setTimeout(() => this.setState({ move: true }), 3000);
    }
     componentWillReceiveProps(){
        this.setState({time: JSON.stringify(Math.round(0.12*this.props.musicInfo*10)/10)});
    }
    ...
    render() {
        return (
            <div  style={{
                ...
                <Motion style={{ x: spring(this.state.move ? this.props.musicInfo : 0) }}>
                    {value => {
                        return <div style={{
                            height: "100%",
                            width: "100%",
                            backgroundColor: "#C9FDE8",
                            position: 'relative',
                            left: '-100%',
