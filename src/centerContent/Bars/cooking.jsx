...
export default class Sleep extends React.Component {
    ...
    componentDidMount() {
        setTimeout(() => this.setState({ move: true }), 2600);
   
    }
     componentWillReceiveProps(){
        this.setState({time: JSON.stringify(Math.round(0.12*this.props.cookingInfo*10)/10)});
    }
    ...
    render() {
        return (
            <div  style={{
                ...
                <Motion style={{ x: spring(this.state.move ? this.props.cookingInfo : 0) }}>
                    {value => {
                        return <div style={{
                            height: "100%",
                            width: "100%",
