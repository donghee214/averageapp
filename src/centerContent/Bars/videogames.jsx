...
export default class Sleep extends React.Component {
    ...
    componentDidMount() {
        setTimeout(() => this.setState({ move: true }), 3200);
    }
   componentWillReceiveProps(){
        this.setState({time: JSON.stringify(Math.round(0.12*this.props.videoGameInfo*10)/10)});
    }
    render() {
        return (
            <div  style={{
                ...
                <Motion style={{ x: spring(this.state.move ? this.props.videoGameInfo : 0) }}>
                    {value => {
                        return <div style={{
                            height: "100%",
                            width: "100%",
